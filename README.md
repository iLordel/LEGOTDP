...
a separate charger profile, an 80% charge limit, live power draw, in English,
Russian and Spanish.

[![Build](https://github.com/iLordel/LEGOTDP/actions/workflows/build.yml/badge.svg)](https://github.com/iLordel/LEGOTDP/actions/workflows/build.yml)
...
- **Live readings** — the limit that is set, the power actually being drawn, the
  SoC temperature and the fan speeds. The first two are different numbers and
  are labelled as such; the third is what tells a firmware thermal cut apart
  from something else moving the limits.
- **Fan modes** — Auto, Quiet, Balanced, Cool, Max. The three curves sit at or
  above the firmware's own minimum, because the reason to touch the fans on a
  handheld is to cool it more than Lenovo chose to, never less. The firmware
  wipes the curve on every power-mode change, so the plugin writes it back and
  stands down after a few refusals. Needs the firmware interface (`acpi_call`);
  the kernel driver publishes fan speeds but not the curve.
...
- **Conflict warnings** — HHD, adjustor, PowerStation, SimpleDeckyTDP,
  ...
  fought.
- **Charge limit** — one switch that stops charging at 80%, the threshold the
  firmware itself uses. Taken through the kernel's own
...
1. Download `LTDP-<version>.zip` from [Releases](https://github.com/iLordel/LEGOTDP/releases)
...
This build targets **N3CN40WW** (January 2026), the BIOS Lenovo currently offers
for the Legion Go 8APU1. Nothing here behaves differently on another version —
the baseline exists so the panel and the diagnostic script can tell you where
you stand.

One release is called out by name: **N3CN42WW** (June 2026) was withdrawn by
Lenovo in August 2026 after handhelds stopped booting after the update. It is no
longer offered for download, and BIOS downgrades on this machine are blocked, so
there is no vendor path back to 40. If you are on 40 or below, do not install it
from a mirror; if you are already on 42 and the machine works, leave it alone.
LTDP flags it in Diagnostics and does nothing else about it.
